...
<style>
#header {background:url(/resources/images/main/bg_nav.png) repeat-x bottom !important;}

.nav {text-align:center;padding:30px 0px 0px 0px;width:600px;margin-left:50px;}
.nav > div > span {margin:0 18px;}
.nav > div > table {margin-top:40px;font-family:'굴림',Gulim;color:#fff;font-size: 12px;}
.nav > div > table td {padding:0px 6px 0px 6px;}
.nav > div > table a {display:inline-block;color:#fff;}

.contTop {text-align: center !important;}
...
(function($){
	...
	var showMenu = function(menuId){
		...
		$('.nav div:eq(0) img').each(function(){
			var strSrc = $(this).attr('src');
			strSrc = strSrc.replace('_on', '_off');
			$(this).attr('src', strSrc);
		});
		$('#header_bottom table').hide();
	...
	var initMenu = function(){
		showMenu();
		if(location.href.indexOf('/searchFormula/view.do') > -1){
			showMenu('menu01');
		}
		if(location.href.substring('') > -1){
			showMenu('menu02');
		}
		if(location.href.substring('') > -1){
			showMenu('menu03');
	...
	$(document).ready(function(){
		...
		$('#menu01').bind('click', function(){
			location.href = '/searchFormula/view.do';
		});
		
		$('#menu11').bind('click', function(){
			location.href = '/project/view.do';
		});
		
		$('#menu12').bind('click', function(){
			location.href = '/user/view.do';
		});
		
		$('#menu21').bind('click', function(){
			location.href = '/admin/view.do';
		});
		
		$('#menu22').bind('click', function(){
